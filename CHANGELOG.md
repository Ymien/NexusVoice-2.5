...
- 🐛 **修复 Action 构建凭证报错 (HTTP 401)**: 彻底解决了在 GitHub Actions 流水线中的 `Bad credentials` 错误。
  - 将 Actions 底层的构建工具从 `v3` 升级至 `v4`。
  - 在打包构建脚本中显式补充了 `mac_certs` 以防止构建容器在进行 macOS (`.dmg`) 签名时由于未识别到证书而回退报错。
  现在 `v0.4.75` 将确保打包流程顺畅并稳定发布。

---

## [v0.4.50] - 2026-04-09
### 修复与优化 (Fixes)
- 🐛 **修复 Git 凭证报错 (HTTP 401)**: 彻底替换了开发环境中过期的 GitHub Token，更新了全新的仓库部署凭证，确保本次提交及后续标签可以成功推送到远端。
- 🎨 **维持精美 UI 与视频模块**: 确认保留并锁定了 `v0.4.25` 中添加的多巴胺配色系统与贾维斯科幻风格流体网络视频组件。
